...
from ..models.time_analysis import AppCategory, HourlyAppUsage, ProductivityType
from ..core.config import settings

logger = logging.getLogger(__name__)

class AppUsageService:
    def __init__(self, db: AsyncSession, es_client: Optional[AsyncElasticsearch] = None):
        self.db = db
        self.es_client = es_client
    
    # 应用分类相关方法
    async def create_app_category(self, name: str, productivity_type: ProductivityType) -> AppCategory:
        """创建新的应用类别"""
        # 检查是否已存在同名类别
    ...
    async def get_app_categories(
        ...
        if productivity_type:
            ...
            count_query = count_query.where(AppCategory.productivity_type == productivity_type)
    ...
    async def delete_app_category(self, category_id: int) -> bool:
        ...
        # 检查是否有关联的应用使用记录
        stmt = select(func.count()).select_from(HourlyAppUsage).where(
    ...
    def get_hourly_app_usage_by_id(self, usage_id: int) -> Optional[HourlyAppUsage]:
        ...
        根据ID获取小时应用使用统计
        """
        return self.db.query(HourlyAppUsage).filter(HourlyAppUsage.id == usage_id).first()
    
    def get_hourly_app_usage_by_user_time_app(self, user_id: str, timestamp: datetime, app_name: str) -> Optional[HourlyAppUsage]:
        """
        根据用户ID、时间戳和应用名称获取小时应用使用统计
        """
        return self.db.query(HourlyAppUsage).filter(
    ...
    async def get_hourly_app_usage(
        ...
        count_query = select(func.count()).select_from(HourlyAppUsage).where(
            and_(
                HourlyAppUsage.usage_date >= start_date,
                HourlyAppUsage.usage_date <= end_date
            )
    ...
    async def get_productivity_summary(
        ...
        query = select(
            ...
            func.sum(HourlyAppUsage.duration_minutes)
        ).join(
            AppCategory,
            HourlyAppUsage.category_id == AppCategory.id
        ).where(
            and_(
                HourlyAppUsage.usage_date >= start_date,
                HourlyAppUsage.usage_date <= end_date
